...
| Container | padding | `.md` (16 pt) вертикальный, `.md` (16 pt) горизонтальный |
| Container | margin | `.md` (16 pt) от краёв sheet, `.sm` (8 pt) bottom |
...
| Title | font | `DS.Typography.title` (= `.title3 .semibold`) |
| Subtitle | text | `L10n.serverAutoSubtitle` = «Выберет лучший по скорости» |
| Subtitle | font | `DS.Typography.subheadline` (= `.subheadline .semibold`) |
...
| Container | padding | `.md` (16 pt) horizontal, `.sm` (8 pt) vertical |
...
| Title | font | `DS.Typography.caption` (= `.caption .regular`) — UPPERCASE через `.textCase(.uppercase)` |
...
| Trailing | font | `DS.Typography.caption` (= `.caption .regular`) |
...
| Элемент | Свойство | Значение |
|---|---|---|
| Container | layout | `HStack(spacing: DS.Spacing.md = 16)` |
| Container | padding | `.md` (16 pt) horizontal, `.md` (16 pt) vertical |
...
| Trailing badge (latency, OK) | font | `DS.Typography.subheadline` (= `.subheadline .semibold`) с `.monospacedDigit()` |
...
| Trailing badge (unsupported) | style | capsule, padding `.sm` h × `.xs` v, background `.tertiarySystemFill`, foreground `.secondary`, font `DS.Typography.caption` (= `.caption .regular`) |
| Trailing badge (unreachable) | text | `L10n.serverList.unreachable` = «недоступен» |
| Trailing badge (unreachable) | font | `DS.Typography.subheadline` (= `.subheadline .semibold`) |
...
| Элемент | Свойство |
|---|---|
| Container | `VStack(spacing: DS.Spacing.md)` centered, padding `.xl` (32 pt) |
| Icon | SF Symbol `tray` size 48 pt, foreground `.tertiary` |
| Title | text `L10n.serverList.emptyTitle` = «Нет серверов», font `DS.Typography.title` (= `.title3 .semibold`) |
| Subtitle | text `L10n.serverList.emptySubtitle` = «Импортируйте конфигурацию через «+» на главном экране.», font `DS.Typography.subheadline` (= `.subheadline .semibold`), `.secondary`, multiline center |
...
### 8.1 Spacing scale — `DS.Spacing` (Phase 3 active set)

Phase 3 использует строго 8-point grid {4, 8, 16, 24, 32, 48, 64}. Все значения кратны 4 pt — соответствует Apple HIG 8-point grid.
...
| `md` | 16 | ServerRow horizontal/vertical padding и HStack spacing, AutoCell internal/external padding, SubscriptionHeader horizontal padding, sheet margins, inline empty card VStack spacing |
| `lg` | 24 | (зарезервирован — не используется в Phase 3 active layout) |
| `xl` | 32 | inline empty card outer padding |
| `xxl` | 48 | (зарезервирован — не используется в Phase 3) |
| `xxxl` | 64 | (зарезервирован — не используется в Phase 3) |

**Exceptions:** none для Phase 3 — каждое значение в scale.

> Phase 2 inherited tokens (если ранее использовались значения 12 pt из устаревшей шкалы) подлежат миграции на нормализованную шкалу в Phase 3 codebase — каждое спецификационное «vertical padding 12» приводится к `md = 16`. Это лёгкий visual delta (+4 pt вертикального воздуха в ServerRow), не требует переработки макетов.
...
### 8.4 Typography — `DS.Typography` (Phase 3 active set)

Phase 3 использует **4 типографических роли** и **2 веса** (`.regular`, `.semibold`). Это строго ограниченный набор для текущей фазы; полная палитра DesignSystem (Phase 11) будет шире, но v0.3 не нуждается в более чем 4 ролях.

| Role | Style | Weight | Design | Phase 3 usage |
|---|---|---|---|---|
| `title` | `.title3` | `.semibold` | `.default` | AutoCell title (§2.3), inline empty card title (§3.5) |
| `body` | `.body` | `.regular` | `.default` | ServerRow title — имя сервера (§2.5) |
| `subheadline` | `.subheadline` | `.semibold` | `.default` | AutoCell subtitle (§2.3), latency badge OK (§2.5), unreachable badge (§2.5), inline empty card subtitle (§3.5) |
| `caption` | `.caption` | `.regular` | `.default` | SubscriptionHeader title с `.textCase(.uppercase)` (§2.4), SubscriptionHeader trailing last-fetched (§2.4), "не поддерживается" pill (§2.5) |

**Веса (только 2):**
- `.regular` — `body`, `caption`
- `.semibold` — `title`, `subheadline`

Body line-height: системный (~1.4–1.5 у Dynamic Type). Не переопределяем (Apple platform consistency).

Latency-числа — `.monospacedDigit()` модификатор поверх `subheadline` role чтобы цифры не дрейфовали (Phase 2 ConnectionTimer pattern). `.monospacedDigit()` не меняет роль/вес — только metric.

**Phase 2 inherited tokens (out of Phase 3 active set):**

| Phase 2 role | Phase 2 style | Phase 3 status |
|---|---|---|
| `display` | `.largeTitle .medium .monospaced` | inherited для ConnectionTimer (Phase 2 экран) — **не используется** в Phase 3 sheet; не считается в Phase 3 typography budget |
| `headline` | `.headline .semibold` | reserved в DesignSystem package — не используется в Phase 3 |
| `callout` | `.callout .regular .rounded` | inherited для ServerLineView (Phase 2) и inline error tooltips — **не используется** новыми Phase 3 компонентами |
| `caption2` | `.caption2 .regular` | **deprecated в Phase 3** — все ранее `caption2` usage'и (last-fetched, unsupported pill) переведены на `caption` role |

> Inherited Phase 2 tokens остаются в DesignSystem package для backward compatibility экранов, не затронутых Phase 3 (MainScreen ConnectionButton/Timer). Phase 3 новые компоненты используют **только** 4 роли из таблицы выше.
...
Где `SectionRowShape` — custom Shape, рендерящий 12 pt corners на top если isFirst, на bottom если isLast, 0 pt в середине. Divider между строками — `Divider().padding(.leading, DS.Spacing.md + 32)` (отступ под флаг).
...
| Typography roles | Phase 11 расширяет до полной палитры (display/headline/callout/caption2 возвращаются) для статистики и других экранов |
...
- [ ] Dimension 4 Typography: PASS — **4 роли** (title/body/subheadline/caption), **2 веса** (.regular/.semibold) через системные TextStyle в §8.4; Phase 2 inherited tokens документированы отдельно
- [ ] Dimension 5 Spacing: PASS — строгий 8-point grid {4, 8, 16, 24, 32, 48, 64} в §8.1, все pt значения кратны 4
...
*UI-SPEC revised: 2026-05-12 (checker block-fix: typography 8→4 roles + 4→2 weights; spacing scale normalized — md=16, removed 12pt non-grid value)*
