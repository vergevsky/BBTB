...
- ✅ **T-A2** CLOSED 2026-05-17 — Verified `buildSingBoxJSON` template paths DEAD в production (callers only в tests + 1 comment в SingBoxConfigLoader). Удалены `XxxInputs` struct + `BuilderError` enum + `buildSingBoxJSON` + `mutatePort` helper + `loadTemplate` helper во всех 6 протоколах (VLESSReality, VLESSTLS, Trojan, Shadowsocks, Hysteria2, TUIC). Tests переписаны с template path на dict-based `buildOutbound` path (4+19+16+4+8+16 = 67 tests passing). Production path остаётся `ConfigImporter` → `PoolBuilder.buildSingleOutboundJSON` (dict + JSONSerialization auto-escape). Template Resource JSON files (SingBoxConfigTemplate.*.json) теперь dead resources — cleanup в Tier C/D.
...
### Cluster 1: Protocols/* Raw JSON Template Substitution ✅ ALL CLOSED 2026-05-17 (T-A2)

(6 findings, all from C8)
...
- **A4-002 / A4-004 / A4-005 / C4-003:** ✅ CLOSED 2026-05-16 (T-A6) — multi-layer size caps: `SubscriptionURLFetcher` switched к streaming `URLSession.bytes(for:)` + 5MB hard cap + Content-Length fast-path reject + new `.bodyTooLarge(Int)` FetchError. `decodeBase64` pre+post-decode caps. `UniversalImportParser.import` rejects rawInput >1MB BEFORE classify. `parseSingBoxJSON` rejects body >5MB + outbounds >200 BEFORE JSONSerialization. Yams.load YAML cap deferred (см. A4-014 MEDIUM, will close в Tier C).
