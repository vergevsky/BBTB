...
**Goal:** Централизованные правила с Ed25519-подписью, split-tunneling по доменам/IP/странам через sing-box `route.rule_set`. Версия — **v0.8**.
**Mode:** mvp
**UI hint:** yes
**Requirements:** CORE-05, RULES-01, RULES-02, RULES-03, RULES-04, RULES-05, RULES-06, RULES-07, RULES-08, RULES-09, RULES-10, ~~RULES-11~~ (Out of Scope per D-08)
**Success Criteria:**
...
3. ~~На macOS AppProxyProvider позволяет роутить отдельные приложения через VPN~~ → **Out of Scope v0.8 per D-08/D-09 (2026-05-15)**, перенесено в v0.10+ conditional. См. `wiki/appproxy-deferral-2026.md` (создаётся в Plan 08-01) + Codex thread `019e284c-4bf6-7f91-ada7-7e679692b5fb`. Workaround: `never_through_vpn` через rule_set domain/IP matching покрывает 95% friends-and-family TestFlight scenarios; per-bundle-ID granularity loss приемлем для MVP.
...
Plans:
**Wave 1**
- [ ] 08-01-PLAN.md — W0 foundation: RULES-11 + Phase 8 SC #3 carve-out (D-08/D-09) — delete AppProxyExtension-macOS target + entitlements; ROADMAP/REQUIREMENTS amendment; promote SubscriptionURLFetcher.isBlockedHost to public; create wiki/appproxy-deferral-2026.md

**Wave 2** *(blocked on Wave 1 completion)*
- [ ] 08-02-PLAN.md — W1 RulesEngine SwiftPM пакет: swift-crypto 4.x + PublicKey + RulesSigner + RulesFetcher (HTTPS+SSRF+mirror failover) + RulesManifest + MockURLProtocol + ≥ 9 unit tests

**Wave 3** *(blocked on Wave 2 completion)*
- [ ] 08-03-PLAN.md — W2 RulesEngineCoordinator pipeline: SRSCacheStore actor + BaselineRulesLoader + RulesSnapshot + bootstrap/performBackgroundRefresh/forceUpdate (60s cooldown) + AppGroupContainer.rulesCacheDirectory + ≥ 13 tests

**Wave 4** *(blocked on Wave 3 completion)*
- [ ] 08-04-PLAN.md — W3 UI: 4 SwiftUI компонента (RulesViewerSection, ForceUpdateRulesButton, MinAppVersionBanner, MinAppVersionSheet) + SettingsViewModel/MainScreenViewModel late-bind wiring + AdvancedSettingsView/MainScreenView modifiers + ~30 L10n keys + ≥ 17 tests

**Wave 5** *(blocked on Wave 4 completion)*
- [ ] 08-05-PLAN.md — W4 host bootstrap: iOS BGAppRefreshTask 6h + macOS NSBackgroundActivityScheduler 6h/10min tolerance + Info.plist + foreground sanity fetch 12h threshold + ViewModel wire-up + iOS/macOS xcodebuild green

**Wave 6** *(blocked on Wave 5 completion)*
- [ ] 08-06-PLAN.md — W5 sing-box engine apply: SingBoxConfigLoader.expandConfigForTunnel injects 3 route.rule_set + 3 priority rules (block→reject, never→direct, always→urltest); idempotent; R1/R10 invariants preserved; + 6 unit tests

**Wave 7** *(blocked on Wave 6 completion)*
- [ ] 08-07-PLAN.md — W6 baseline assets: scripts/build-baseline-rules.sh (sing-box CLI compile + openssl Ed25519 sign) + baseline-rules.json source + real signed Resources (3 .srs + 4 .sig + manifest)

**Wave 8** *(blocked on Wave 7 completion)*
- [ ] 08-08-PLAN.md — W7 closure: validate-r1-r6.sh extended с Phase 8 invariants (R8 + RULES-02 + D-08); wiki sync (rules-engine.md, architecture.md, security-gaps.md, log.md); STATE/PROJECT update; iOS+macOS xcodebuild regression gate + all package swift test green
...
- [x] 10-03-PLAN.md — STUN block injection в SingBoxConfigLoader (шаг 6, между rule_set и Mux) + macOS enforceRoutes path: PlatformHooks/KillSwitch читают App Group UserDefaults + SettingsViewModel.applyEnforceRoutesToManager() live-apply + ≥ 7 unit-тестов (BIO-04, KILL-04)
