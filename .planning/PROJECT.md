...
---
*Last updated: 2026-05-15 — Phase 10 ✅ **Complete** (Advanced Settings + Security Polish). DPI-05 Mux injection, DPI-06 CDN fronting (`FrontingEngine` SwiftPM package), DPI-08 cert pinning (`PinStore/PinnedSessionDelegate/SubscriptionPinManager`), BIO-04 STUN block, KILL-04 macOS enforceRoutes toggle, UX-06 uTLS picker. 6 plans, 534 tests, 0 failures. Code review: 5 criticals fixed (force-unwrap, SSRF guard, redundant ModelContext, KillSwitch App Group ID); DPI-08 production pins → Phase 12. DPI-06 end-to-end activation → Phase 11 (admin handoff). Human UAT deferred: UI layout, STUN alert, macOS enforceRoutes live-apply, Mux device log. Next: Phase 11 (Onboarding + UX polish).*

---

*2026-05-18 update — Phase 13 v0.13 Internal TestFlight UNBLOCKED.* Plan 09 audit fix-up cycle (23 PRs merged, 26 findings closed — 13 HIGH + 9 MEDIUM + 4 LOW) полностью addressed AUDIT-4 «External TestFlight BLOCK» scope. External Rollout path tracking — `.planning/BACKLOG.md` §10 Tier 1 (3 of 5 remaining: real Ed25519 + signed manifest infra, Phase 9 Wave 4 AASA deploy, device UAT). `Wiki/rules-engine.md` § «Закрытые / принятые решения» records PublicKey owner clarification. Session paused 2026-05-18 12:30 на Tier 1 #2 closure (libbox log privacy PR #24 commit `d46034c`); resume на Tier 1 #1 Ed25519 key generation. См. `.planning/STATE.md` § «Session-pause resume guide».
