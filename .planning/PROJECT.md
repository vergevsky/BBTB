...
---
*Last updated: 2026-05-14 — Phase 6d ✅ **Closed** после UAT regression smoke PASS на iPhone iOS 26.5 (commit `cff3f46` + closure commit `d777548`). 19 findings closed + 7 post-fix correctness commits (cold-start UI freeze block + Settings-disable saga finalized via ExternalVPNStopMarker + Apple-canonical `options["manualStart"]`). 6 architectural patterns DEC-06d-01..06 установлены (см. R19). PERF-01..05 + QUAL-01..03 промоутнуты в Validated. 26 carved-out findings → Phase 6e backlog. Phase 6c R18 — re-confirmed (`44a5630` Round 6 fix). Next focus: `/gsd-discuss-phase 7` (Anti-DPI suite + WireGuard family, v0.7).*
