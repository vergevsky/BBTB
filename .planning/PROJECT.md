...
- **WireGuard plain (PROTO-06)** — Out of Scope, v1.x backlog conditional on TestFlight demand _(Phase 7 discuss 2026-05-14 D-02; ТСПУ blocks plain WG behaviorally since Feb 2026; UDP closed in RU summer 2025; AmneziaWG 2.0 покрывает WG-нишу; см. `wiki/wireguard-deferral-2026.md`)_
- **OpenVPN/TLS (PROTO-09)** — Out of Scope, v1.x backlog conditional on TestFlight demand _(Phase 7 discuss 2026-05-14 D-01; ТСПУ blocks OpenVPN полностью с Feb 2026; OpenVPN+Cloak phased out from Amnezia Premium 2026; sing-box не умеет OpenVPN, требует second engine Partout + GPLv3 commercial licensing; см. `wiki/openvpn-deferral-2026.md`)_
- **AmneziaWG v1/v1.5 back-compat** — Out of Scope для MVP; Phase 7b ships **v2.0 only**; conditional return на TestFlight demand
...
| **R20: Phase 7 — РФ 2026 reality + split 7a/7b + engine abstraction + anti-DPI smart defaults** (decided 2026-05-14 после `/gsd-discuss-phase 7` deep research с Codex GPT-5 ×3 advisory threads + WebSearch по статусу VPN-протоколов в РФ май 2026) | По итогам deep research (ТСПУ behavioral fingerprinting since Dec 2025, full block OpenVPN+plain-WG since Feb 2026, 469 заблокированных VPN-сервисов март 2026, ML-algorithms интегрированы март 2026; Amnezia Premium phased out OpenVPN-over-Cloak в 2026, реальный РФ-фронтир = VLESS+Reality / AmneziaWG 2.0 / Hysteria2 / TUIC) — 5 архитектурных решений: **D-01** PROTO-09 OpenVPN/TLS → Out of Scope, v1.x backlog conditional on demand (sing-box не умеет OpenVPN, требует Partout engine с GPLv3 + commercial для AppStore, ТСПУ blocks полностью); **D-02** PROTO-06 plain WireGuard → Out of Scope, v1.x backlog conditional (ТСПУ blocks behaviorally, AmneziaWG 2.0 покрывает нишу); **D-03** PROTO-07 AmneziaWG **2.0 only** через `amneziawg-apple` SwiftPM library (MIT, форк wireguard-apple) + первый multi-engine integration в проекте с engine abstraction layer в одном `NEPacketTunnelProvider` extension; **D-04** Phase 7 разделена на 7a (v0.7.1, sing-box-only — TUIC v5 + anti-DPI smart defaults) и 7b (v0.7.2, engine abstraction + AmneziaWG 2.0) с отдельными TestFlight-релизами и iPhone UAT-циклами; **D-05** anti-DPI smart defaults — uTLS=random автоматически для всех TLS-протоколов, `tls.record_fragment: true` ON для VLESS+TLS/Trojan (Codex Q4 follow-up: НЕ для TUIC — QUIC «only ECH»; НЕ для Reality/Vision — XTLS), mux OFF по умолчанию (ломает Vision/Reality), URI overrides всегда уважаются. Reframes: DPI-04 random delay → covered by AmneziaWG junk packets (не отдельное sing-box требование); DPI-03 packet padding → mux-layer padding только при mux per-server. CONTEXT.md — `.planning/phases/07-anti-dpi-suite-wireguard-family/07-CONTEXT.md`. Wiki decision logs: `wiki/openvpn-deferral-2026.md`, `wiki/wireguard-deferral-2026.md`, `wiki/amneziawg-integration.md` (последняя — будет создана в Phase 7b). | ✅ **Phase 7a Closed 2026-05-14** — iPhone UAT PASS на Trojan subscription (commits `8ca1014` + `1d98abc` + `cb6140b` + `49c40d5` + closure). PROTO-08 / DPI-01 / DPI-02 / DPI-07 → Validated. Wave 3 (mux infra) → Phase 10 unified PR. TUIC connection device-UAT carved-out. **Phase 7b in planning** — discuss-phase для engine abstraction + AmneziaWG 2.0. |
...
---
*Last updated: 2026-05-14 — Phase 7a ✅ **Closed** после iPhone UAT PASS на Trojan subscription. PROTO-08 (TUIC v5) + DPI-01 (uTLS=random) + DPI-02 (`tls.record_fragment`) + DPI-07 (port diversity) → Validated. Wave 3 (mux infra) → Phase 10 unified PR с DPI-09 UI picker. TUIC connection device-UAT carved-out (conditional на появление TUIC сервера). Phase 7b discuss-phase starting — engine abstraction + AmneziaWG 2.0 (v0.7.2). См. R20.*
