...
---
*Last updated: 2026-05-13 (evening) — Phase 6c ✅ **Closed** после re-UAT PASS pair на iPhone iOS 26.5 + Round 6 follow-up fix (commit `44a5630`). NET-08..11 промоутнуты `[ ] Active` → `[x] Validated`. Apple's on-demand reconnect + reactive UI driver + VM foreground resync — production-ready. NET-12 (active liveness probe) — carve-out для Phase 7-8. Next focus: proposed Phase 6d (Performance & Code Quality Audit, multi-AI peer review) перед Phase 7 (Anti-DPI + WireGuard).*
