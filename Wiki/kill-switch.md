---
name: Kill switch
description: Системный kill switch через includeAllNetworks, по умолчанию ВЫКЛЮЧЕН, тоггл в Расширенных
type: project
---

# Kill switch

**Summary**: Системный kill switch через `includeAllNetworks = true` в `NEVPNProtocol`. Если туннель падает, ОС блокирует весь сетевой трафик до восстановления или ручного отключения. **По умолчанию выключен** (изменено 2026-05-12 — снижение friction при первом запуске).
...
- **Выключен по умолчанию** (`@AppStorage("app.bbtb.killSwitchEnabled") = false`) — решение 2026-05-12. Пользователь не видит блокировку интернета при первом запуске если что-то пошло не так.
- Тоггл для включения/отключения — в разделе [[ux-specification|Расширенных]] настроек
...
- ✓ **v0.1** — kill switch технически реализован (с `enforceRoutes=true`); по умолчанию был включён
- ✓ **v0.2** — тоггл «выключить kill switch» в Безопасность + ReconnectBanner при активном KS
- ✓ **v0.3+** (2026-05-12) — default изменён на **выключен**: пользователь сам решает, включать ли kill switch; снижает friction при первом запуске
