...
- [x] **✓ `libbox.writeDebugMessage` privacy level switch (closed 2026-05-18 PR #24 commit `d46034c`).** Plan 07 Q5 user decision executed перед External Rollout: `writeDebugMessage` message + `send(notification:)` title/body privacy `.public` → `.private` в `ExtensionPlatformInterface.swift`. DiagnosticsExporter user-facing log export НЕ затронут (reads `AppGroupContainer.singBoxLogPath` напрямую). Codex Reviewer thread `019e3a46` APPROVE. См. memory `feedback_libbox_log_privacy_external_rollout.md` (✓ COMPLETED status).
