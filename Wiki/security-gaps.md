...
| `stack` | `"mixed"` | Hiddify-app DEFAULT для iOS (verified via gh-api на `hiddify-core/v2/config/hiddify_option.go`). Старый выбор `gvisor` (Phase 1 W3) и комментарий «mixed требует raw sockets, NE sandbox запрещает» оказались **неверными** — Hiddify годами shipped mixed на iOS через `gomobile bind -target ios` с tags `with_gvisor,with_low_memory,with_purego`. Round-3 device debug 2026-05-11 показал: с `gvisor` все соединения умирают за <500мс одинаково; mixed — battle-tested путь (system stack для TCP via fd-based netstack + gvisor для UDP). |
