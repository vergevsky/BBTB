...
**Summary**: Долгосрочная память о производительности приложения BBTB после Phase 6d (Performance & Code Quality Audit, закрыта 2026-05-14). Triple-AI peer review (Claude Opus 4.7 + Codex GPT-5.2 + Gemini 3.1 Pro) → 45 findings, 19 закрыто, 26 carved-out в backlog. Cold-start ~−500…−1100 мс, connect-tap ~−1000…−3000 мс, disconnect −2.5 сек, плюс корректность и energy-win. Numerical Instruments baseline осознанно skipped (Variant D — user-выбранный trade-off скорости перед UAT).
...
**Last updated**: 2026-05-14 (Phase 6d закрытие)
...
## Open follow-ups (carved-out backlog — 26 findings)

| Severity | Count | IDs | Disposition |
|---|---|---|---|
| **MEDIUM (carved)** | 6 | M6, M7, M8, M10, M11, M15 | User-impact unclear без Instruments baseline OR fix-effort high vs expected delta; revisit Phase 6e «Performance Audit Round 2» |
| **LOW** | 20 | L1-L20 | Code quality / minor cleanup; backlog forever или low-effort bundle Phase 6e |
| **Trivial imports** | 3 | L-trivial-imports | 3-line cleanup candidate для Phase 6e bundle |

Полный детальный список — `.planning/phases/06d-performance-audit/06D-FINDINGS.md` (git-tracked).

**Дополнительные open items вне FINDINGS:**
- **NET-12 — active liveness probe** (Phase 6c carve-out, не закрыт в 6d). Pitfall 5: tunnel `.connected` но трафик не идёт (soft kill server-side sing-box). См. `.planning/REQUIREMENTS.md:113`.
- **macOS-specific UAT replay** — Phase 6c/6d сценарии A/F/Settings-disable/G не выполнялись на macOS отдельно (только scenario C). Risk низкий (тот же source code), но для production strength — отдельная UAT-сессия перед Phase 11/12.
- **Numerical Instruments baseline (post-Phase-6d capture)** — опционально снять single capture на физическом устройстве *сейчас* (post-fix) как baseline для будущих регрессий. PerfSignposter готов (DEC-06d-06).
- **`L18` — ServerListViewModel lazy init в MainScreenViewModel** — carved separately, не Phase 6d scope.
