...
## 2026-05-16 (late evening, hardening pass) — Post UI fix-loop technical closure

5 commits на main (`d52dc13` → `25bfda6`) — закрывают Phase 12 carve-outs +
полируют UX, который проявился после UI fix-loop.

**Commits chronologically:**

1. `d52dc13 docs(12): UI fix-loop COMPLETE — wiki + STATE sync`
   - `wiki/swift-pixel-perfect-rebuild-2026.md` секция «2026-05-16 (late) —
     User-driven UI fix-loop» с patterns + per-screen
   - `wiki/log.md` 9-commit chronological list
   - `.planning/STATE.md` status `awaiting-uat` → `ui-fix-loop-complete`

2. `20f0d78 refactor(ui): migrate inline TopBars → BBTBTopBar component`
   - MainScreenView TopBar inline HStack → BBTBTopBar(leading: menuButton, trailing: addMenu)
   - ServerListSheet header inline HStack → BBTBTopBar(title:, trailing: refreshBtn)
   - ServerDetailView inline back TopBar → BBTBTopBar(title:, onBack:) convenience init
   - ~60 строк дублирования устранено; будущие правки header — в одном месте

3. `94e7b78 test(12): record 11 snapshot baselines + linker fix`
   - `ServerListFeatureTests` + `SettingsFeatureTests` получили
     `linkerSettings: [.linkedLibrary("resolv"), .linkedLibrary("bsm", .when(macOS)),
     .linkedFramework("SystemConfiguration", .when(macOS)), .linkedFramework("AppKit"),
     .linkedFramework("UIKit")]`
   - 11 baselines записаны: 5 ConnectionButton (idle/connecting/connected/error/regular)
     + 1 OnboardingView + 5 ServerList (autoCell × 2, serverRow × 2, sheet corners)
   - Verify run TEST SUCCEEDED 11/11 за 27s

4. `7f60783 fix(test): snapshot tests render в Dark mode`
   - `.preferredColorScheme(.dark)` → `.environment(\.colorScheme, .dark)` (11×)
   - `.preferredColorScheme` это hint для presentation containers (sheet,
     fullScreenCover); для standalone snapshot rendering нужен direct
     Environment value override
   - 11 baselines re-recorded с правильным Dark trait — controlIdle #222
     dark gray, canvas #000 black, alwaysWhite text на accent/error

5. `25bfda6 fix(ux): recompute MainScreen state on ServerListSheet dismiss`
   - `.sheet(isPresented:onDismiss:)` теперь зовёт `viewModel.refresh()` async
   - Закрытие sheet → recompute supported count → state .idle → .empty
     когда `supported.isEmpty` → EmptyStateCard вместо ConnectionButton
   - Phase 11 D-01 sticky-forever `hasShownOnboarding` preserved

**Изменённые wiki страницы:**
- `wiki/swift-pixel-perfect-rebuild-2026.md` — добавлена секция «Hardening
  (post UI fix-loop)»; «Deferred» backlog обновлён (BBTBTopBar migration done,
  snapshot baselines done; остались Subscription quota + visual UAT)

---

## 2026-05-16 (late evening, Phase 13 setup) — Transition to TestFlight Internal Distribution

User decision: «Мне достаточно Internal testers» — стартуем Phase 13 через
TestFlight Internal Testing only (до 100 testers, friends-and-family beta).

**Phase 12 → 13 transition:**
- Phase 12 «Swift pixel-perfect rebuild» — CLOSED (commit `33c34b4` UAT signoff)
- Phase 13 «TestFlight Internal Distribution» — PLANNING

**Decisions locked (`.planning/phases/13-testflight-internal-distribution/13-CONTEXT.md`):**
- D-01 Internal Testing only — skip Beta App Review + Privacy Policy URL + full App Store metadata
- D-02 SPKI pinning deferred to v1.1+ (verified dead code, commit `eb44740`)
- D-03 Apple Distribution = Xcode automatic signing

**Project config verified:**
- Team ID: `UAN8W9Q82U`
- Bundle IDs: `app.bbtb.client.ios` + `app.bbtb.client.ios.tunnel`
- Code signing: Automatic

**Open prerequisites (5):**
1. Verify Apple Developer Program active
2. Network Extension capability на 2 App IDs в Apple Portal
3. App Store Connect record creation
4. Xcode Archive → Upload (auto-managed signing)
5. Export Compliance one-time answer

**Carry-forward to v1.1+:**
- External Testing + Privacy Policy URL + public invite link
- SPKI pin replacement (real cert hashes via `scripts/generate-spki-pin.swift`)
- DETECT-03 admin handoff
...
**Modified files:**
- `wiki/distribution-testflight.md` — Internal-only strategy + full walkthrough section
- `.planning/STATE.md` — milestone `v0.12` → `v0.13`, status `planning`
- `.planning/phases/13-testflight-internal-distribution/13-CONTEXT.md` — new
