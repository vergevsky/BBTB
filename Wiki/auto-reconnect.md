...
**Last updated**: 2026-05-13 (Phase 6c — **Wave 3 ✓ Complete на main + re-UAT PASS**, commits `19f3fe7` + `5b0e28c` + `69b8ae8` + `44a5630`; AppFeatures 133/133 + xcodebuild iOS + macOS green; re-UAT pair F-reverse + Settings-disable + G passive all PASS on iPhone iOS 26.5 after Round 6 follow-up fix `44a5630`).
...
### VM foreground resync (Round 6 fix, commit `44a5630`)

Re-UAT 2026-05-13 вскрыл UI-desync edge case: пользователь отключает BBTB через **iOS Settings → VPN → toggle off**. Системный VPN выключается (intent-closing в TunnelController срабатывает), но UI приложения зависает на `.connected(since:)` с тикающим таймером.

**Root cause** (Codex GPT-5.2 architect, advisory pass): `MainScreenViewModel.nevpnStatusObserver` был зарегистрирован с `queue: .main`. Во время Settings round-trip приложение в background, main queue приостановлена → `.disconnected` notification **coalesced/dropped** и **не replays** на возврат. TunnelController observer не пострадал — он использует `queue: nil` (выполняется на любой очереди). У VM не было foreground-resync хука (на iOS `tc.handleForeground()` — no-op).

**Архитектурное решение — 3 surgical changes:**

1. **VM observer queue `.main → nil`** — Task { @MainActor } hop внутри callback сохраняет main-actor мутации `@Published`. Независимость от main-queue scheduling устраняет drop при suspended app.
2. **`MainScreenViewModel.handleForeground()`** — одна XPC-поездка на scene `.active`: `loadAllFromPreferences` → `ManagerSelector.ourManagers` фильтр (защита от чужих VPN) → read `connection.status` + `connection.connectedDate` (sync, без XPC) → `applyVPNStatus(_:connectedDate:)`. Transient XPC failures → keep last state.
3. **scenePhase wiring** — `BBTB_iOSApp.swift` + `BBTB_macOSApp.swift` вызывают `viewModel.handleForeground()` рядом с существующим `tc.handleForeground()` на `.active`.

**Что preserved:** intent-closing в TunnelController UNCHANGED (F-reverse stays PASS); no XPC в NEVPNStatusDidChange observer (G — Mach-port safety); no reintroduction of removed machinery; `applyVPNStatus` остаётся SINGLE authority. Новая XPC-поездка — одна на scene `.active`, не в hot loop; укладывается в Phase 6 budget «≤1 XPC per significant event».

### Bonus: `connectedDate` authority для `since` (commit `44a5630`)

Параллельный edge case: пользователь активирует BBTB через **iOS Settings → VPN → toggle ON** (не через app), затем открывает приложение через час. Старая логика писала `state = .connected(since: Date())` — таймер начинал с момента foreground open, не с реального connect.

**Fix:** `applyVPNStatus(_:)` принимает опциональный `connectedDate: Date?` (default nil — обратная совместимость с тестами). `.connected` ветка использует `connectedDate ?? state.connectionStart ?? Date()`. Observer + `handleForeground` читают `conn.connectedDate` (sync property, без XPC) и передают.

Authority order для `since`:
1. **NE-reported real start** (`NEVPNConnection.connectedDate`) — авторитет.
2. **Sticky existing `state.connectionStart`** — если уже в `.connected`, сохраняем (защищает от рандомного перезатирания тем же значением).
3. **`Date()`** — финальный fallback (первое появление `.connected` без known date — почти невозможный edge).

Таймер теперь стартует с реального момента установления туннеля независимо от того, откуда пришло подключение (BBTB-тап / iOS Settings / on-demand re-evaluator).
