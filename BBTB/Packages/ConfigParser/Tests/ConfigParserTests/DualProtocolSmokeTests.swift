...
final class DualProtocolSmokeTests: XCTestCase {
    ...
    // MARK: Test 2 — mixed with unsupported → only supported go to pool

    func test_multiline_withUnsupported_isolatesSupportedForPool() async throws {
        let input = """
    ...
    // MARK: Test 4 — only unsupported → PoolBuilder throws

    func test_onlyUnsupported_poolBuilderThrows() async throws {
        let input = """
        ss://abc@host:8388#SS
        ...
        XCTAssertEqual(result.unsupported.count, 2)
